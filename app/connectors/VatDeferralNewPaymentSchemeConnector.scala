...
class VatDeferralNewPaymentSchemeConnector @Inject()(http: HttpClient,
  ...
  def eligibility(vrn: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[Boolean]] = {
    val url = s"$serviceURL/vat-deferral-new-payment-scheme/eligibility/$vrn"
    http.GET[Eligibility](url).map{
      case Eligibility(Some(true),_,_,_,_)           => Some(true)
      case Eligibility(_,_,_,Some(false),Some(true)) => Some(false)
      case _ => None
    }
  }.recover{
    case e: Exception =>
