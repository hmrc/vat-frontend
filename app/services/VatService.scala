...
trait VatServiceInterface {
  def fetchVatModel(
      vatEnrolment: VatDecEnrolment)(implicit headerCarrier: HeaderCarrier, request: Request[_]): Future[Either[VatAccountFailure, Option[VatData]]]
...
class VatService @Inject() (vatConnector: VatConnector)(implicit ec: ExecutionContext) extends VatServiceInterface with Logging {
  ...
  def fetchVatModel(
    ...
    vatEnrolment match {
        ...
        vatConnector
          ...
          .flatMap {
            case Some(accountSummary) =>
              vatCalendar(enrolment).map { calendarDerivedInformation =>
                Right(
                  Some(VatData(accountSummary, calendarDerivedInformation.map(_.calendar), calendarDerivedInformation.map(_.outstandingReturnCount))))
              }
      ...
      case VatDecEnrolment(_, false) => Future.successful(Left(VatUnactivated))
    }
  ...
  private def determineFrequencyFromStaggerCode(staggerCode: String): FilingFrequency = {
    ...
    staggerCode match {
      ...
      case _ =>
        logger.warn(s"The user has an invalid stagger code of $staggerCode")
        InvalidStaggerCode
    }
  }

  def vatCalendar(
      vatEnrolment: VatEnrolment)(implicit headerCarrier: HeaderCarrier, request: Request[_]): Future[Option[CalendarDerivedInformation]] =
    vatConnector.calendar(vatEnrolment.vrn).map {

      case Some(calendarData @ CalendarData(Some(staggerCode), directDebit, _, _)) =>
        val filingFrequency   = determineFrequencyFromStaggerCode(staggerCode)
        val directDebitStatus = DirectDebitStatus.from(directDebit)
        Some(CalendarDerivedInformation(Calendar(filingFrequency, directDebitStatus), calendarData.countReturnsToComplete))
