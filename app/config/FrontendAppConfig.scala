...
class FrontendAppConfig @Inject() (override val runModeConfiguration: Configuration, environment: Environment) extends ServicesConfig with PortalUrlBuilder {
  ...
  lazy val businessAccountWrongCredsUrl: String = businessAccountHost + loadConfig("urls.business-account.wrongCreds")
  lazy val addVatUrl: String = addTaxHost + loadConfig("urls.add-tax.addVat")
