...
class AccountSummaryHelperSpec extends ViewSpecBase with MockitoSugar with ScalaFutures {
  ...
  "there is an account summary to render and account is in credit" should {
    ...
    "not have Direct Debit information when direct debit is eligbile and inactive, but the user files annually" in {
    ...
    "not have Direct Debit information when direct debit is eligbile and active, but the user files annually" in {
